...
public class CharactorController : MonoBehaviour
{
    ...
    void Initialize()
    {
        ...
        for (int i = 0; i < 3; i++)
        {
            do
            {
                beforeCharaPrefab.Initialize(CharaCsvLoader.beforeChangeCharaDatas[Random.Range(0, CharaCsvLoader.beforeChangeCharaDatas.Count)]);
            } while (beforeCharaPrefab.ID == charaIDs[0] ||
                   beforeCharaPrefab.ID == charaIDs[1] ||
                   beforeCharaPrefab.ID == charaIDs[2]);

            charaIDs[i] = beforeCharaPrefab.ID;
            beforeChangeCharacters.Add(Instantiate(beforeCharaPrefab));
        }
    }

    void CharaGenerate(int _generatePosY)
    {
        do
        {
            beforeCharaPrefab.Initialize(CharaCsvLoader.beforeChangeCharaDatas[Random.Range(0, CharaCsvLoader.beforeChangeCharaDatas.Count)]);
        } while (beforeCharaPrefab.ID == beforeChangeCharacters[0].ID ||
               beforeCharaPrefab.ID == beforeChangeCharacters[1].ID ||
               beforeCharaPrefab.ID == beforeChangeCharacters[2].ID);

        beforeChangeCharacters[_generatePosY] = Instantiate(beforeCharaPrefab);
    }
